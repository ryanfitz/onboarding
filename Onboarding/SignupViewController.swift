...
//            navigationBar.shadowImage = self.imageWithColor(UIColor.darkGrayColor())
    ...
    func layoutSubviews() {
        ...
        foregroundContentView.setTranslatesAutoresizingMaskIntoConstraints(false)
        foregroundContentScrollView.setTranslatesAutoresizingMaskIntoConstraints(false)
    ...
    func textFieldShouldReturn(textField: UITextField) -> Bool {
        ...
        for (index, view) in enumerate(textFields) {
            if (view.labeledTextField == textField && (textFields.count > index + 1)) {
                ...
                if let nextView = self.textFields[index + 1] as CNHFloatLabeledTextFieldView? {
